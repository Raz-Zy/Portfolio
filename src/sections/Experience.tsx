...
const experienceLinks: (string | undefined)[] = [undefined, undefined, undefined, undefined]
...
const cardGradients = [
  ...
  'linear-gradient(135deg, #00CEC9 0%, #0E7490 100%)',
]
  ...
  return (
    <section className="py-20">
      ...
      {/* Sticky card deck, full page width: each card pins below the navbar
          ...
          position:sticky, no scroll hijacking. On small screens the cascade
          offset is tighter (and the card styles more compact) so a whole
          card still fits the viewport. */}
      <div className="px-3 md:px-6">
        {m.experience.items.map((item, index) => (
          <div
            key={index}
            className="sticky mb-6 last:mb-0 top-[calc(76px+var(--deck-i)*10px)] md:top-[calc(88px+var(--deck-i)*36px)]"
            style={{ '--deck-i': index } as React.CSSProperties}
          >
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="text-center text-xs md:text-sm tracking-[0.2em] uppercase text-white/70 mb-4 md:mb-10">
                ({item.type})
              </div>
              ...
              <div className="flex items-start justify-between gap-6 flex-1">
                <div>
                  <h3 className="text-2xl md:text-5xl font-bold leading-tight max-w-2xl">
                    {item.title}
                  </h3>
                  <p className="text-lg md:text-2xl text-white/85 font-semibold mt-2 md:mt-3">
                    {item.company}
                  </p>
                  <div className="flex flex-wrap gap-x-6 gap-y-2 mt-3 md:mt-5 text-xs md:text-sm text-white/75">
                ...
                <span
                  aria-hidden="true"
                  className="text-5xl md:text-8xl font-bold text-white/80 leading-none shrink-0"
                >
              ...
              <div className="mt-6 md:mt-12">
                <div className="grid md:grid-cols-2 gap-x-12 gap-y-2.5 md:gap-y-4">
                  {item.description.map((desc, descIndex) => (
                    <div
                      key={descIndex}
                      className="pt-2.5 md:pt-4 border-t border-white/30 flex items-start gap-3 text-xs md:text-base text-white/90"
                    >
                ...
                {item.technologies && (
                  <div className="flex flex-wrap gap-2 mt-4 md:mt-8">
                    {item.technologies.map((tech, techIndex) => (
                      <span
                        key={techIndex}
                        className="px-3 py-1 bg-white/15 backdrop-blur-sm text-white rounded-full text-xs md:text-sm"
                      >
                        {tech}
                      </span>
                    ))}
